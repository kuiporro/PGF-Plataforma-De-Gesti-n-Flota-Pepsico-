...
export default function EditDriver() {
    ...
    const load = async () => {
      try {
        ...
        setForm({
          ...
          vehiculo_asignado: data.vehiculo_asignado?.id || "",
          km_mensual_promedio: data.km_mensual_promedio || "",
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!r.ok) {
        toast.error(data.detail || "Error al actualizar chofer");
        if (data.errors) {
  ...
  return (
    <RoleGuard allow={["ADMIN", "SUPERVISOR", "JEFE_TALLER"]}>
      <div className="p-8 max-w-3xl mx-auto">
        <h1 className="text-3xl font-bold mb-6 text-gray-900 dark:text-white">Editar Chofer</h1>
