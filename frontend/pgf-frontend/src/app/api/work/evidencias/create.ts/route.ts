...
export async function POST(req: NextRequest) {
  try {
    ...
    const r = await fetch(`${backend}/api/v1/work/evidencias/`, {
      method: "POST",
      headers: {
    ...
    console.error("Error in /api/work/evidencias/create:", error);
    return Response.json(
      { detail: "Failed to create evidencia", error: String(error) },
      { status: 500 }
    );
  }
}
