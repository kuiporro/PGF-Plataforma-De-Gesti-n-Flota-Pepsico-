...
export async function POST(req: NextRequest) {
  try {
    ...
    const r = await fetch(`${backend}/api/v1/work/evidencias/presigned/`, {
      method: "POST",
      headers: {
    ...
    console.error("Error in /api/work/evidencias/presigned:", error);
    return Response.json(
      { detail: "Failed to get presigned URL", error: String(error) },
      { status: 500 }
    );
  }
}
