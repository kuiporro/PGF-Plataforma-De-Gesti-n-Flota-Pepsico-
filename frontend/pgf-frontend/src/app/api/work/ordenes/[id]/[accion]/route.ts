...
export async function POST(req: NextRequest, context: any) {
  try {
    ...
    const r = await fetch(`${backend}/api/v1/work/ordenes/${id}/${accion}/`, {
      method: "POST",
    ...
    console.error("Error in /api/work/ordenes/[id]/[accion]:", error);
    return Response.json(
      { detail: "Failed to execute action", error: String(error) },
      { status: 500 }
    );
  }
}
